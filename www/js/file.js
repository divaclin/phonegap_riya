  ...
  createFile:function(){
    window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function (fs) {
        fs.root.getFile(fileData.path(), { create: true, exclusive: false }, function (fileEntry) {
            fileData.export(fileEntry, '{height:"",phoneHeight:"",mode:"ground",latitude:"",longitude:"",altitude:"",alpha:"",space:[],inertia:[]}');// fileEntry.name == 'someFile.txt' // fileEntry.fullPath == '/someFile.txt'
    ...
    fileEntry.file(function (file) {
        ...
        reader.onloadend = function() {
            try{
              ...
              glAllData = {height:"",phoneHeight:"",mode:"ground",latitude:"",longitude:"",altitude:"",alpha:"",space:[],inertia:[]};
              if(sessionStorage.getItem('index')==null
               ||sessionStorage.getItem('index')==undefined
               ||sessionStorage.getItem('index')==''){
                 sessionStorage.setItem('index',0);
...
function fileCalculate(){
  ...
  for(let i=0;i<space.length;i++){
    ...
    if(space[i][0]['real']['mode']==='ceiling' && space[i][0]['real']['cx']==0){
       space[i][0]['real'] = getDistance(glAllData.height,glAllData.phoneHeight,space[i][0]['a'],space[i][0]['b'],'ground');
       space[i][1]['real'] = getDistance(glAllData.height,glAllData.phoneHeight,space[i][1]['a'],space[i][1]['b'],'ground');
       space[i][0]['real']['mode'] = 'ceiling';
       space[i][1]['real']['mode'] = 'ceiling';
       let CeilingToPhoneBottom = getCeilingToPhoneBottom(glAllData.height,glAllData.phoneHeight,space[i][0]['a'],space[i][1]['b'],space[i][0]['real']['distance']);

       hasCeiling = true;
       for(let j=2;j<space[i].length;j++){
         space[i][j]['real'] = getCeilingDis(glAllData.height,glAllData.phoneHeight,space[i][j]['a'],space[i][j]['b'],CeilingToPhoneBottom);
       }

       let range = getRange(space[i]);
       let re    = remap(space[i],range);
       let ceilingHeight = getCeilingHeight(glAllData.height,glAllData.phoneHeight,space[i][0]['real']['distance'],space[i][1]['b']);
       draw2D(plat.getContext('2d'),plat,re);
       draw3D(poly,space[i],ceilingHeight);
    }
    else if(space[i][0]['real']['mode']==='ceiling' && space[i][0]['real']['cx']!=0){
        let range = getRange(space[i]);
        let re    = remap(space[i],range);
        let ceilingHeight = getCeilingHeight(glAllData.height,glAllData.phoneHeight,space[i][0]['real']['distance'],space[i][1]['b']);
        draw2D(plat.getContext('2d'),plat,re);
        ...
        let range = getRange(space[i]);
        let re    = remap(space[i],range);
        let height= getCeilingHeight(glAllData.height,glAllData.phoneHeight,space[i][0]['real']['distance'],space[i][1]['b']);
        draw2D(plat.getContext('2d'),plat,re);
