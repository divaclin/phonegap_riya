		...
		Router.prototype.page = function(){
          ...
          let body   = '<article style="background:#fafafa;">'+
                          '<center class="indexCenter" style="margin-top:50px;">'+
                           ...
                           '<label style="color:#000;">機高:<input id="define_mobile_height" type="text" value="'+glAllData.phoneHeight+'" /></label><br>'+
                           '<label style="color:#000;">記錄模式:<select class="form-control" id="define_mode"><option '+(glAllData.mode==='ground'?'selected':'')+' value="ground">地面</option><option '+(glAllData.mode==='ceiling'?'selected':'')+' value="ceiling">天花板</option></select></label>'+
...
function setHeight(){
  ...
  glAllData.mode = $('#define_mode').val();
  fileData.export(glFileEntry,JSON.stringify(glAllData));
  window.location.href = "#index";
}
